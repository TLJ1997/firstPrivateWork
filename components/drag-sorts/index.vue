...
export default {
  ...
  computed: {
	  boxHeight(){
		  return (Math.ceil((Number(this.list.length)+1)/3)) * this.height + 'px'  
	  }
  },
  props: {
    list: {
  ...
  created () {
	...
	this.windowWidth = res.windowWidth - (-40)
    this.onUpdateCurrentList()
  },
  mounted () {
  },
  ...
  methods: {
    onUpdateCurrentList (list = this.list) {
      ...
      for (const key in list) {
		  // console.log(key)
		  let height =  Math.ceil((Number(key)+1)/3) - 1
		  let x = 0
		  if(key <= 2){
			  x = key * this.windowWidth * 0.24 + this.windowWidth * 0.04 || this.windowWidth * 0.04
		  }else{
			  if((Number(key)+1)%3 === 0){
				   x = 2 * this.windowWidth * 0.24 + this.windowWidth * 0.04 || this.windowWidth * 0.04
			  }else{
				  x = ((Number(key)+1)%3-1) * this.windowWidth * 0.24 + this.windowWidth * 0.04 || this.windowWidth * 0.04
			  }
	...
	moveUpdateCurrentList(index){
      for (const i in this.currentList) {
		  ...
		  let temobj = { ...this.currentList[i] }
		   // debugger
		  this.currentList[i].y =  (Math.ceil((Number(key)+1)/3) - 1)*this.height 
		  if(index == key){
			  continue
		  }else{
			  if(key <= 2){
				  ...
				  if((Number(key)+1)%3 === 0){
					   this.currentList[i].x = 2 * this.windowWidth * 0.24 + this.windowWidth * 0.04 || this.windowWidth * 0.04
				  }else{
					   this.currentList[i].x = ((Number(key)+1)%3-1) * this.windowWidth * 0.24 + this.windowWidth * 0.04 || this.windowWidth * 0.04
				  }
			  } 
		  }
      }
...
.drag-sort-item {
  ...
  box-sizing: border-box;
   flex-wrap: wrap;
  width: 180upx;height: 60upx;
  /* margin-left: 10upx; */
  border-radius: 30upx;
  .item {
	position: relative;
	flex: 1;
	line-height: 60upx;text-align: center;border-radius: 30upx;flex-wrap: wrap;
	background:rgba(192,196,204,.2);
	text{
		color: #8792A8;font-size:22upx;
	}
  }
  .close{
	  position: absolute;
