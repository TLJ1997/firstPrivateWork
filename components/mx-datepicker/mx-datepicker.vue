	...
	<view v-if="isShow" class="picker">
		...
		<view v-if="type!='time'" class="picker-modal">
			...
			<view class="picker-modal-footer">
				<view class="picker-modal-footer-info">
					<block v-if="isMultiSelect">
						<view class="picker-display">
							<text>{{beginText}}{{$t('datepicker.rq')}}</text>
						...
						<view class="picker-display">
							<text>{{endText}}{{$t('datepicker.rq')}}</text>
				...
				<view class="picker-modal-footer-btn">
					<view class="picker-btn" :hover-stay-time="100" hover-class="picker-btn-active" @click="onCancel">{{$t('datepicker.qx')}}</view>
					<view class="picker-btn" :style="{color}" :hover-stay-time="100" hover-class="picker-btn-active" @click="onConfirm">{{$t('datepicker.qd')}}</view>
				</view>
			</view>
		</view>
		<!-- 时间选择器 -->
		<view v-if="showTimePicker" class="picker">
			<view class="picker-modal picker-time">
				<view class="picker-modal-header">
					<text class="picker-modal-header-title">{{$t('datepicker.xzrq')}}</text>
				</view>
				...
				<view class="picker-modal-footer">
					...
					<view class="picker-modal-footer-btn">
						<view class="picker-btn" :hover-stay-time="100" hover-class="picker-btn-active" @click="onCancelTime">{{$t('datepicker.qx')}}</view>
						<view class="picker-btn" :style="{color}" :hover-stay-time="100" hover-class="picker-btn-active" @click="onConfirmTime">{{$t('datepicker.qd')}}</view>
					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	const DateTools = {
		...
		getHoliday(date) {
			let holidays = {
				'0101': this.$t('datepicker.yd'),
				'0214': this.$t('datepicker.qr'),
				'0308': this.$t('datepicker.fn'),
				'0312': this.$t('datepicker.zs'),
				'0401': this.$t('datepicker.yr'),
				'0501': this.$t('datepicker.ld'),
				'0504': this.$t('datepicker.qn'),
				'0601': this.$t('datepicker.ed'),
				'0701': this.$t('datepicker.jd'),
				'0801': this.$t('datepicker.jj'),
				'0903': this.$t('datepicker.ks'),
				'0910': this.$t('datepicker.js'),
				'1001': this.$t('datepicker.gq'),
				'1031': this.$t('datepicker.ws'),
				'1224': this.$t('datepicker.pa'),
				'1225': this.$t('datepicker.sd')
			};
	...
	export default {
		props: {
			...
			beginText: {
				type: String,
				default:  this.$t('datepicker.ks')
			},
			//结束文案 针对type为范围选择时生效
			endText: {
				type: String,
				default:  this.$t('datepicker.js')
			}
		},
		data() {
			return {
				...
				if (this.type == 'time') {
					...
					if (this.isMultiSelect) {
						...
						if (this.checkeds.length < 2) return uni.showToast({
							icon: 'none',
							title:this.$t('datepicker.qxzlgrq')
						});
		...
		computed: {
			BeginTitle() {
				let value = this.$t('datepicker.wxz');
			...
			BeginTimeTitle() {
				return this.BeginTitle != this.$t('datepicker.wxz') ? DateTools.formatTimeArray(this.beginTime, this.showSeconds) : '';
			},
			EndTimeTitle() {
				return this.EndTitle != this.$t('datepicker.wxz') ? DateTools.formatTimeArray(this.endTime, this.showSeconds) : '';
			}
		},
		watch: {
