	...
	<view class="container hi-columns" :style="backgroundALL == '#1c1f22'?'background:#18253D;':'background:#F0F3F5;'">
		...
		<view class="header_title_box hi-columns hi-flex-vcenter">
			<view class="header_item hi-rows hi-flex-vcenter" style="align-self: flex-start;">
				<text class="t1" :style="hb_status?'color: #242833;font-size: 48upx;':'color: #ffffff;font-size: 48upx;'">{{$t('wo.ghsjh')}}</text>
			...
			<view class="header_item hi-columns" style="align-self: flex-start;margin-top: 40upx;">
				<text style="font-size: 48upx;color: #3A74FC;text-align: left;">{{Phone.MobilePhone}}</text> 
				<text class="t1" style="font-size: 24upx;color: #8792A8;align-self: flex-start;margin-left: 20upx;">{{$t('wo.yzmzls')}}</text>
			</view>
		</view>
		...
		<view class="hi-columns" style="margin-top: 60upx;padding: 0 60upx;box-sizing: border-box;">
			...
			<view class="input_item hi-rows hi-flex-vcenter" :style="hb_status?'border-bottom: 1upx solid rgba(36,41,51,.1);margin-bottom: 30upx;':'border-bottom: 1upx solid rgba(255,255,255,.1);margin-bottom: 30upx;'">
				<input type="number" value="" v-model="yzm" @input="yzmChange" :placeholder="input_code" placeholder-style="color:#8792A8;" :style="hb_status?'color: #242833;':'color: #ffffff;'" />
					 <text class="yzm_btn" @click="getVCode()" :style="Phone.status?'color: #4966F5;':''">{{vcodeBtnName}}</text>
			</view>


			<view class="btns_box" @click="Login_btn()" :style="Phone.status&&yzm!=''?'background: #3A74FC;color: #FFFFFF;':''">{{$t('wo.tj')}}</view>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			this.Phone.diqu=e.diqu;
			this.Phone.diqunum=e.diqunum;
		...
		methods: {
			...
			Login_btn(){
				...
				if(this.Phone.status&&this.yzm!=''){
					...
					console.log(this.Phone.MobilePhone)
					console.log(this.Phone.diqu)
					console.log(this.Phone.diqunum)
