<template>
	<view class="container hi-columns" :style="backgroundALL == '#1c1f22'?'background:#18253D;':'background:#F0F3F5;'">
		...
		<view class="header_title_box hi-columns hi-flex-vcenter">
			<view class="header_item hi-rows hi-flex-vcenter" style="align-self: flex-start;">
				<text class="t1" :style="hb_status?'color: #242833;font-size: 48upx;':'color: #ffffff;font-size: 48upx;'">{{$t('wo.xgdlmm')}}</text>
	...
	<!-- 		<view style="display: inline-flex; width: 100%; position: relative;">
				<view style="width:40%; height: 70upx; line-height: 70upx; font-size: 35upx;">选择验证方式</view>
			...
			<view class="hi-rows hi-flex-vcenter" :style="hb_status?'border-bottom: 1upx solid rgba(36,41,51,.1);margin-bottom:80upx;':'border-bottom: 1upx solid rgba(255,255,255,.1);margin-bottom:80upx;'">
			
				<input type="text" v-model="xiayibu" :placeholder="input_phone"
			...
			<view class="input_item hi-rows hi-flex-vcenter" style="margin-top: 40upx;" :style="hb_status?'border-bottom: 1upx solid rgba(36,41,51,.1);margin-bottom:40upx;':'border-bottom: 1upx solid rgba(255,255,255,.1);margin-bottom: 40upx;'">
				...
				<view style="width:60%;">
					<input type="text" v-model="yanzhengma" :style="hb_status?'color: #242833;':'color: #ffffff;'" style="width: 100%; height:85upx; line-height:85upx; text-align: left; "
					 :placeholder="input_code" />
				</view>
				<button type="primary" :disabled="btn_state" @tap="fasongma()" style="width:40%; background: #007AFF; font-size: 30upx;height: 80upx; border-radius: 50upx; line-height: 80upx; margin-top:15upx; ">{{yzm_text}}</button>
			</view>
			<view class="input_item hi-rows hi-flex-vcenter" :style="hb_status?'border-bottom: 1upx solid rgba(36,41,51,.1);margin-bottom: 80upx;':'border-bottom: 1upx solid rgba(255,255,255,.1);margin-bottom: 80upx;'">
				...
				<input type="number" password="true" value="" v-model="passwd1" @input="passwd1Change" :placeholder="newPassword"
				 placeholder-style="color:#8792A8;" :style="hb_status?'color: #242833;flex:1;':'color: #ffffff;flex:1;'" />
			...
			<view class="input_item hi-rows hi-flex-vcenter" :style="hb_status?'border-bottom: 1upx solid rgba(36,41,51,.1);margin-bottom: 80upx;':'border-bottom: 1upx solid rgba(255,255,255,.1);margin-bottom: 80upx;'">
				...
				<input type="number" password="true" value="" v-model="passwd2" @input="passwd2Change" :placeholder="input_confirm"
				 placeholder-style="color:#8792A8;" :style="hb_status?'color: #242833;flex:1;':'color: #ffffff;flex:1;'" />
				<image :src="hb_status?'../../../static/home/nav_icon_close_day.png':'../../../static/home/nav_icon_close_night.png'"
...
<script>
	...
	export default {
		data() {
			return {
				input_phone:this.$t('wo.qsrsjyxh'),
				input_code:this.$t('wo.qsryzm'),
				newPassword:this.$t('wo.qsrxmm'),
				input_confirm:this.$t('wo.qqrxmm'),
		...
		methods: {
			baocun(zhanghao) {
				if (this.xiayibu == '') {
					this.myAlert(this.$t('wo.qsrsjyxh'))
					return false
				}
				if (this.yanzhengma == '') {
				...
				if (this.passwd1 != this.passwd2) {
					this.myAlert(this.$t('wo.lcmmbyzqcxsrxmm'))
					return false
				}
				uni.showLoading({
					title: this.$t('wo.xgz'),
				...
				var quanbu = {
					account: this.xiayibu,
					verifyCode: this.yanzhengma,
					newPwd: this.passwd1
				}
				console.log(quanbu)
				uni.request({
					url: this.websiteUrl + 'user/forgetPwd',
					method: "POST",
					data: quanbu,
					header: {
			...
			fasongma() {
				...
				var Regx = /^[A-Za-z0-9]*$/;
				if (Regx.test(this.xiayibu)) {
					console.log('手机')
					this.fs_yzm(this.xiayibu)

				} else {
					console.log('邮箱')
					this.youxiang_yzm(this.xiayibu)
