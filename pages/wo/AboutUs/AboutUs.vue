	...
	<view class="container hi-columns" :style="backgroundALL == '#1c1f22'?'background:#18253D;':'background:#F0F3F5;'">
		...
		<view class="header_title_box hi-columns hi-flex-vcenter">
			<view class="header_item hi-rows hi-flex-vcenter" style="align-self: flex-start;">
				<text class="t1" :style="hb_status?'color: #242833;font-size: 48upx;':'color: #ffffff;font-size: 48upx;'">{{$t('wo.gywm')}}</text>
		...
		<view class="hi-columns" style="padding: 40upx;box-sizing: border-box;">
			<view class="item_bb hi-columns hi-flex-center hi-flex-vcenter" :style="hb_status?'background: #FFFFFF;':'background: #18253D;'">
				<image src="../../../static/logo.png" mode="" style="width: 160upx;height: 160upx;"></image>
				<text class="t1" :style="hb_status?'color: #242933;':'color:#FFFFFF'">{{$t('wo.bb')}} 2.2.1</text>
				<text class="t2">YouKeycoin  {{$t('wo.bqsy')}}</text>
			</view>
		</view>
		<navigator class="items hi-rows hi-flex-vcenter hi-space-between" :style="hb_status?'background: #FFFFFF;':'background: #18253D;'" url="/pages/wo/AboutUs/Aboutjs">
			<text class="ts" :style="hb_status?'color: #242933;':'color:#FFFFFF'">{{$t('wo.jysjs')}}</text>
			<text  class="grace-icons icon-arrow-right"></text>
		</navigator>
		<navigator @tap="openXiyi({name:'免责协议',type:'mianze'})" class="items hi-rows hi-flex-vcenter hi-space-between" :style="hb_status?'background: #FFFFFF;':'background: #18253D;'">
			<text class="ts" :style="hb_status?'color: #242933;':'color:#FFFFFF'">{{$t('wo.yhxy')}}</text>
			<text  class="grace-icons icon-arrow-right"></text>
		</navigator>
...
<script>
	...
	export default {
		...
		methods: {
			...
			openXiyi(content){
				uni.navigateTo({
					url:'../xieyi?name='+content.name+'&type='+content.type
				})
			},
			Back() {
				uni.navigateBack({ //后退
...
<style>
	...
	.item_bb{width: 100%;height: 400upx;border-radius:20upx;margin-bottom: 30upx;}
	.item_bb .t1{font-size: 28upx;color: #242933;margin-top: 20upx;}
	.item_bb .t2{font-size: 24upx;color: #8792A8;margin-top: 12upx;}
