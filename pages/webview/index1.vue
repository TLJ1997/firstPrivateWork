<template>
	<div style='background-color:#101f32;height: 100%;width: 100%;overflow: hidden;position: relative;' >
		...
		<view class="button-sp-area" style="position: absolute;">
			<view class="mini-btn" @click="ChangeKLinePeriod(KLINE_PERIOD_ID.KLINE_MINUTE_ID)">1{{$t('webview.fen')}}</view>
			<view class="mini-btn" @click="ChangeKLinePeriod(KLINE_PERIOD_ID.KLINE_15MINUTE_ID)">15{{$t('webview.fen')}}</view>
			<view class="mini-btn" @click="ChangeKLinePeriod(KLINE_PERIOD_ID.KLINE_30MINUTE_ID)">30{{$t('webview.fen')}}</view>
			<view class="mini-btn" @click="ChangeKLinePeriod(KLINE_PERIOD_ID.KLINE_60MINUTE_ID)">60{{$t('webview.fen')}}</view>
			<view class="mini-btn" @click="ChangeKLineIndex(0,'MA')">MA</view>
			<view class="mini-btn" @click="ChangeKLineIndex(0,'BOLL')">BOLL</view>
			<view class="mini-btn" @click="ChangeKLineIndex(2,'MACD')">MACD</view>
			<view class="mini-btn" @click="ChangeKLineIndex(2,'RSI')">RSI</view>
			<view class="mini-btn" @click="ChangeKLineIndex(2,'KDJ')">KDJ</view>
		</view>
		<view style="width:46upx;height: 46upx;position: absolute; bottom:30upx; right: 30px;">
			<image :src="hb_status?'../../static/home/buquanping.png':'../../static/home/buquanping.png'" mode=""
		 style="width:46upx;height: 46upx;" @click="toALLDisplay(2)"></image>
		 </view>
	</div>
</template>
	...
	onLoad(values) {
		...
		uni.getSystemInfo({
		    success: function (res) {
		        //提取屏幕宽度
				that.ChartWidth = res.windowWidth-40;
				that.ChartHeight = res.windowHeight-40;
				console.log(res)
		    }
		});
	},
